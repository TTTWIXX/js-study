...
if (bullet > 0 && bullet <= 5) {
    ...
    rd = Math.floor(Math.random() * bulletCase.length)
    member[startIndex] = rd;


    prompt('엔터를 누르면 격발합니다. Its high noon');
    // 첫번째순서부터 격발
    if (bulletCase[member[startIndex]] === 1) {
        alert('BBang!')
        ...
        alert(`운좋네`)
        bulletCase=bulletCase.pop();
    }

    if(member[startIndex+1]===undefined){
        alert(`${member[0]} 님의 턴 탄창을 회전합니다.`)
        rd = Math.floor(Math.random() * bulletCase.length)
        member[0]=rd
        if(bulletCase[ member[0]]===1){
            alert('BBang!')
        bulletCase=bulletCase.shift();
         ...
         alert(`${member[startIndex+1]} 님의 턴 탄창을 회전합니다.`)
    }
